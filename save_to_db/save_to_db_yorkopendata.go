...
func main() {
	...
	// define collection
	collection := client.Database("cvst_pubsub").Collection("york_open_data")
	...
	go func() {
		for msg := range msgs {
			open_data := string(msg.Body)
			fmt.Println("Reading")
			fmt.Println(open_data[:100])
			// bson.D{{"Data", open_data}}
			_, err := collection.InsertOne(context.TODO(), bson.D{{"Data", open_data}})
			if err != nil {
				fmt.Println(err.Error())
				os.Exit(1)
			}
